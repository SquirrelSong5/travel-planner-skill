...
完整行程默认逐一尝试五类来源，但按平台属性分阶段使用，不做无目的的重复搜索。详细优先级、证据字段与降级策略见 [references/data-sources.md](references/data-sources.md)。

| 阶段 | 来源 | 平台职责 | 必须产出 |
| --- | --- | --- | --- |
| 目的地发现 | 小红书 | 发现近期分区、节奏、排队和踩雷 | 候选区域与体验风险 |
| 规则确认 | 官方与运营方 | 确认开放、预约、票务、临时通知、天气和交通规则 | 决定能不能去的硬事实 |
| 路线落地 | 高德地图 | 确认 POI、坐标、距离、路线和通勤时长 | 可执行的地点顺序与移动时间 |
| 餐饮筛选 | 美团攻略 | 围绕已确定区域建立餐厅候选池 | 菜系、预算和用餐场景候选 |
| 交易核对 | 携程等国内 OTA | 核对酒店、机票、火车、门票价格和可订状态 | 价格区间、预订状态与国内入口 |

在 `source_coverage` 中为五个平台各写一项，记录 `status`、`stages`、`purpose`、`checked_at` 和 `source_refs`。无法访问或确实不适用时仍保留该平台项，用 `unavailable` / `not_applicable` 和 `note` 说明原因，不得假装已使用。执行顺序不代表可信度；发生冲突时仍以最新官方结论为准。
...
- 某个来源不可用：走合规降级，并在 `source_coverage` 记录状态、替代方法和影响，不伪造已使用痕迹。
...
先用小红书发现目的地分区、近期节奏与风险，再用官方渠道决定候选是否可用；候选收敛后用高德落路线，用美团围绕当日区域找餐厅，最后用 OTA 核对交易与价格。不要让攻略先于官方做最终决定，也不要在区域未确定前搜索大量餐厅。

交付前回到官方、高德和 OTA 复核最易变化的开放、路线、价格与可售状态。小红书和美团只重查会影响当前选择的体验信号。
...
对话版必须直接使用 `trip.md` 的内容，不要另写一份可能漂移的攻略。打开 HTML 检查标题、日期、地图、五类信息源覆盖、预订状态、Plan B、安全提醒、行前复核、预算和移动端布局。高德 Web Key 只允许用户在页面内输入并保存到当前设备；不得通过 `?k=` 参数传递。
