...
### 硬约束 2：必跑 MCP（不能凭 LLM 记忆）

- ❌ **不允许** AI 凭训练数据**写** POI 名称 / 地址 / 营业时间 / 评分 / 路线 / 通勤时间 / 餐厅推荐 / 酒店信息——**这些数据必须通过 MCP 实时查询**
...
> - ✅ **输出 JSON 结构**：`maps_direction_walking` 返回的 JSON 结构和 `paths[].steps[].path` polyline 格式在 7 个客户端都一样
>
...
- `examples/tokyo-4n5d.json` — 境外参考 demo（东京 5 日）
