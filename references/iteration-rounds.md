...
**前置**：Step 1.5 已完成 `xhs_destination_brief`（未完成不得进 Round 1）。

1. 对照 brief 的 `must_visit` / `skip_or_caution` 删点；天数不够的远郊、节假日爆款远郊、用户禁忌点 → 写入 `deleted[]`（明说删了什么 + 为什么）
2. 清单分组：城内轻松 / 预约 / 远郊 / 可路过（布局参考 brief 的 `region_layout`）
3. 草案：每天主区域 + 锚点 POI + 粗时间块（**不**调高德 route，只用直线距离预筛）
4. `must_visit` 未纳入 POI 池的，critique 须说明原因
5. 酒店锚点是否覆盖最多天数（🤖 AI 审查）
6. V7 用户禁忌：方案中不得出现禁忌类型 POI
