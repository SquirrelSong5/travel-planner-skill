...
| 挂载位置 | 字段 | 时间轴 |
|---------|------|--------|
| `pois[]` | `price` | 独立 `price-card` |
| `transports[]` | `fare` | `next-leg` 下 `price-card` |
| `meals.*.main` | `price` | `meal-leg` 下 `price-card` |
| `hotel` / `prebook[]` | `price` | 仅抽屉「花销预估」 |

- Step 1 必收 `party_size`
- Round 2 写 `transports[].fare`（与 direction MCP 同调用）
- Round 3 写 POI/meal `price`（与 `maps_search_detail` 同调用）
