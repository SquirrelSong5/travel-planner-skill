...
**默认**：步行（近）→ 骑行（中短）→ 地铁/公交（较长）→ 打车（仅备选）。

| 场景 | 首选 | 备选 |
|------|------|------|
| < 1.5km | 步行 | — |
| **1.5–4km** | **`maps_bicycling` 骑行** | 公交（无骑行道/雨天） |
| **4–25km 市内** | `maps_direction_transit_integrated` | 骑行（仅用户明确要） |
| 机场/火车站 ↔ 市区 | 机场快线 / 地铁（查高德公交方案） | 打车（仅无末班公交、深夜抵达、或用户明确要） |

**抵达日写法**：Day 1 首 POI 可以是机场/车站；须调 **`maps_direction_transit_integrated`** 补 **机场 → 酒店** 段（`from_idx=机场.idx, to_idx=0`），`description` 从 API `segments` 拼出（如「步行 → 地铁8号线 → 地铁3号线 → 步行」）。胶东机场等有站内地铁时**通常不必先打车**——以 API 返回为准；若 `segments[].taxi` 非空再写「打车接驳」。
| 远郊无公交 | 驾车/打车 | 在 `note` 写明「无公交，建议包车/打车」 |

**选酒店时**：优先地铁 500m 内、换乘 ≤1 次能到当日主区域（见 `hotel-planning.md`）。

**写进 JSON**：
- `transports[].mode` 用 `walking` / `biking` / `transit` / `subway` / `bus`，少用 `driving`
- `description` 写清线路印象（如「地铁 2 号线 → 1 号线，约 25 分钟」），勿只写「打车 8 分钟」
- 若必须用打车，在 `note` 或 `description` **一句话说明原因**（无公交 / 用户要求 / 深夜）

**反模式**：
- ❌ 市内 3km 直接 `maps_direction_driving`
- ❌ 酒店 `commute` 全写「打车」而不查地铁
- ❌ 2km 可骑行却硬塞地铁换乘
...
在分组前排线前，先搜「{城市} {N}天 攻略」「{城市} 避雷」等，精读 3–5 篇，产出 `xhs_destination_brief`（必去/避雷/分区/节奏）。并行 Web search 补闭馆与预约。详见 `xhs-research.md` §0。
...
- **地点名**：导航 URL 中的名称用 **`nav_name`（可选）** 或自动从 `name` 去掉「入住 / 退房 / 取行李」等活动后缀；酒店相关 POI 优先用 `trip.hotel.name`。
- **不要**把行程标题（如「某某酒店入住」）原样塞进导航参数。
...
2. **市内公共交通优先** —— 默认地铁/公交，打车须有事由
3. **计划 = 参考坐标** —— 天气、位置、体力、饥饿均可覆盖
4. **基准价值** —— 给 agent 和用户清楚定位：今日区域、锚点、可放弃点、补给点
