...
| 类型 | 正确来源 | 可接受的降级 |
|------|---------|-------------|
| 机票 / 高铁 | 高德 MCP `maps_flight_*` 或航司官网实查 | 携程/飞猪**带搜索词**的 URL，如 `https://flights.ctrip.com/online/list/oneway-{from}-{to}?...` |
| 酒店 | 高德 MCP 酒店详情 / Booking 搜索 | `https://hotels.ctrip.com/hotels/list?city={cityId}&keyword={酒店名}` |
| 景点门票 | 官方小程序/公众号说明 + 大众点评 POI | `https://www.dianping.com/search/keyword/{cityId}/{景点名}` |
| 无直链 | 写 `null`，不要编 | 模板会降级为 Bing 搜索 |
