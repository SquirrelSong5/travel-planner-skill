...
- 完整攻略默认逐一尝试官方、高德、国内 OTA、小红书和美团五类来源；
- 按平台属性划分目的地发现、规则确认、路线落地、餐饮筛选、交易核对和交付前复核；
- 新增 `source_coverage`，记录每个平台的使用状态、阶段、用途、核对时间和证据链接；
- V0 阻止遗漏或重复的平台覆盖记录，以及不符合平台职责的阶段；
- Markdown 与 HTML 同步展示五源实际使用情况；无法访问或不适用时必须说明原因。
...
- 将 SKILL 主文档从客户端/版本百科收敛为能力驱动的规划工作流；
- 明确 5 类来源：高德、官方与网页搜索、国内 OTA、小红书、美团攻略；
- 大众点评移出默认链路，公开发布改为用户确认后的可选步骤；
- 新增证据优先级、隐私边界、核心 JSON Schema 与 OpenAI UI 元数据。

### 安全与可靠性

- 修复 HTML 注入/XSS，限制酒店链接协议；
- 地图 Key 不再从 URL 参数读取，只保存在当前浏览器；
- 修复携程国内域名识别与硬编码航线兜底；
- `add_hotel_legs.py` 不再默认覆盖输入文件；
- `seed_prices.py` 的演示估算不再伪装成真实来源。

### 校验与测试

- 新增 V0 核心结构检查，强化 V3/V5/V8/V10/V13；
- 新增 `--fail-on-warn`；
- 路线 URL 测试改为仓库内可复现的参数化脚本；
- CI 运行编译、13 项单元测试、严格示例校验、路线与 HTML smoke test。
